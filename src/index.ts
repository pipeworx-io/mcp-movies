...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Movies');
}
...
async function searchMovies(query: string, limit: number) {
  ...
  const res = await pwFetch(`${ITUNES_BASE}/search?${params}`);
  if (!res.ok) throw await httpError(res, 'iTunes Search error');
...
async function searchTvShows(query: string) {
  ...
  if (!res.ok) throw await httpError(res, 'TVmaze error');
...
async function getTvShow(id: number) {
  const res = await pwFetch(`${TVMAZE_BASE}/shows/${id}?embed=episodes`);
...
async function getTvSchedule(country: string, date?: string) {
  ...
  const res = await pwFetch(`${TVMAZE_BASE}/schedule?${params}`);
  if (!res.ok) throw await httpError(res, 'TVmaze error');
